...
public:
  explicit LoopClosure(const LoopClosureConfig &config) {
    ...
    ////// quatro init
    quatro_handler_ = std::make_shared<quatro<PointType>>(
        qc.fpfh_normal_radius_, qc.fpfh_radius_, qc.noise_bound_,
        qc.rot_gnc_factor_, qc.rot_cost_diff_thr_, qc.quatro_max_iter_,
        qc.estimat_scale_, qc.use_optimized_matching_,
        qc.quatro_distance_threshold_, qc.quatro_max_num_corres_);
  ...
  int fetchClosestKeyframeIdx(const PosePcd &front_keyframe,
    ...
    for (size_t idx = 0; idx < keyframes.size() - 1; ++idx) {
      ...
      double tmp_dist = (keyframes[idx].pose_corrected_eig_.block<3, 1>(0, 3) -
                         front_keyframe.pose_corrected_eig_.block<3, 1>(0, 3))
                            .norm();
      if (loop_det_radi > tmp_dist &&
          loop_det_tdiff_thr <
              (front_keyframe.timestamp_ - keyframes[idx].timestamp_)) {
  ...
  PcdPair setSrcAndDstCloud(const std::vector<PosePcd> &keyframes,
    ...
    if (enable_submap_matching) {
      for (int i = src_idx - submap_range; i < src_idx + submap_range + 1;
           ++i) {
        if (i >= 0 && i < static_cast<int>(keyframes.size() - 1)) {
          src_accum +=
              transformPcd(keyframes[i].pcd_, keyframes[i].pose_corrected_eig_);
        }
      }
      for (int i = dst_idx - submap_range; i < dst_idx + submap_range + 1;
           ++i) {
        if (i >= 0 && i < static_cast<int>(keyframes.size() - 1)) {
          dst_accum +=
              transformPcd(keyframes[i].pcd_, keyframes[i].pose_corrected_eig_);
      ...
      src_accum = transformPcd(keyframes[src_idx].pcd_,
                               keyframes[src_idx].pose_corrected_eig_);
      if (enable_quatro) {
        dst_accum = transformPcd(keyframes[dst_idx].pcd_,
                                 keyframes[dst_idx].pose_corrected_eig_);
      } else {
        // For ICP matching,
        ...
        for (int i = dst_idx - submap_range; i < dst_idx + submap_range + 1;
             ++i) {
          if (i >= 0 && i < static_cast<int>(keyframes.size() - 1)) {
            dst_accum += transformPcd(keyframes[i].pcd_,
                                      keyframes[i].pose_corrected_eig_);
          }
        }
      }
    }
    return {*voxelizePcd(src_accum, voxel_res),
  ...
  RegistrationOutput
  coarseToFineAlignment(const pcl::PointCloud<PointType> &src,
                        const pcl::PointCloud<PointType> &dst) {
    RegistrationOutput reg_output;
  ...
  RegistrationOutput performLoopClosure(const PosePcd &query_keyframe,
    ...
    if (closest_keyframe_idx_ >= 0) {
      ...
      // keyframe's neighbor)
      const auto &[src_cloud, dst_cloud] = setSrcAndDstCloud(
          keyframes, query_keyframe.idx_, closest_keyframe_idx_,
  ...
  pcl::PointCloud<PointType> getSourceCloud() { return *src_cloud_; }

  pcl::PointCloud<PointType> getTargetCloud() { return *dst_cloud_; }

  pcl::PointCloud<PointType> getCoarseAlignedCloud() { return coarse_aligned_; }

  pcl::PointCloud<PointType> getFinalAlignedCloud() { return aligned_; }

  int getClosestKeyframeidx() { return closest_keyframe_idx_; }
};
